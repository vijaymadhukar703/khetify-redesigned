import { editMeasurementError, editMeasurementPayload, measurementEditState } from './measurementEdit.js';
...
import { createAdminProduct, updateAdminProduct, getAdminProduct, checkAdminDuplicateName, searchHsn, getGstByHsn } from '../../lib/adminProductApi';
...
const BLANK = {
  // Admin library only — the company this product is listed on behalf of.
  company_name: '',
  legal_name: '',
  lic_no: '',
  cin_no: '',
...
/**
 * ADMIN PRODUCT LIBRARY → upload / edit form. A copy of the seller's
 * pages/seller/SellerMyProductForm.jsx (left untouched) wired to
 * lib/adminProductApi.js, plus four company identity fields at the top of
 * Basic Product Information: Company Name, Legal Name, LIC No., CIN No.
 * HSN lookups here go through /api/hsn with the admin token (see
 * adminProductApi.js) — the seller notes below describe the original form.
 *
 * ── Original notes (seller form) ──
 ...
 */
const AdminProductForm = ({ productId = null, onCancel, onSaved }) => {
      ...
      .then((r) => {
        ...
        if (rows.length) {
          ...
          setVariantRows(rows.map(v => ({
            ...measurementEditState(v),
            measurements: (v.measurements ?? v.dimensions) ? { ...(v.measurements ?? v.dimensions) } : undefined,
            label: v.label,
  ...
  const validate = () => {
    ...
    if (!formData.legal_name.trim()) return 'Please enter the Legal Name.';
    const cin = formData.cin_no.trim();
    if (cin && cin.length !== 21) return 'Enter a valid 21-character CIN No.';
  ...
  const handleSubmit = async (e) => {
    ...
    const fields = {
      ...
      legalName: formData.legal_name.trim(),
      // Sent even when blank so clearing them on edit clears the saved value.
      licNo: formData.lic_no.trim(),
      cinNo: formData.cin_no.trim().toUpperCase(),
    ...
    try {
      const res = isEdit ? await updateAdminProduct(productId, data) : await createAdminProduct(data);
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input id="company_name" className={inputClass} value={formData.company_name} onChange={handleInputChange} placeholder="e.g., Green Valley Agro Traders" required />
              <p className="mt-1.5 text-xs text-stone-400">The business this product is listed on behalf of. For attribution only.</p>
            </div>
            <div>
              <label className={labelClass}>Legal Name <span className="text-[#EA2831]">*</span></label>
              <input id="legal_name" className={inputClass} value={formData.legal_name} onChange={handleInputChange} placeholder="e.g., Green Valley Agro Traders Pvt. Ltd." required />
            </div>
            <div>
              <label className={labelClass}>LIC No.</label>
              <input id="lic_no" className={inputClass} value={formData.lic_no} onChange={handleInputChange} placeholder="Enter licence number" />
            </div>
            <div>
              <label className={labelClass}>CIN No.</label>
              <input
                id="cin_no"
                className={inputClass}
                value={formData.cin_no}
                onChange={(e) => setFormData(prev => ({ ...prev, cin_no: e.target.value.toUpperCase() }))}
