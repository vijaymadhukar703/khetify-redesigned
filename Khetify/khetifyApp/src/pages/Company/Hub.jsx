...
import {
  ManifestModal,
  ReceiveModal,
} from "../../Components/ims/TransferModals";
...
function companyNotifRoute(n) {
  ...
  if (
    kind === "supply_request" ||
    kind.startsWith("supply_") ||
    n?.type === "supply_status"
  )
    return "/supply-requests";
  if (kind === "transfer_incoming" || n?.type === "shipment")
    return "/operations?tab=shipments";
...
function transferLabel(status) {
  if (["planned", "approved", "loading"].includes(status))
    return "Awaiting dispatch";
  if (["in_transit", "arrived", "verifying"].includes(status))
    return "Dispatched";
  if (["received", "partially_received", "delivered"].includes(status))
    return "Delivered";
  return status;
}
...
const Hub = () => {
  ...
  // Phone verification banner ka data. Yahi getCompany call se aata hai jo
  // status ke liye pehle se ho rahi hai — koi naya request nahi.
  //
  // `null` ka matlab "abhi pata nahi" hai, `false` se alag: jab tak jawab na
  // aa jaye banner nahi dikhta, warna har refresh par ek pal ko chamak jata
  // — us user ko bhi jiska number verified hai.
  ...
  useEffect(() => {
    ...
    getCompany(companyId)
      .then((c) => {
        if (!alive) return;
        setCompanyStatus(c?.status ?? null);
        setNumberVerified(c?.numberVerified ?? false);
  ...
  // Banner tabhi jab jawab aa chuka ho AUR number verified na ho. Jiske paas
  // number hi nahi, use verify karne ko kehna bemaani hai.
  const needsPhoneVerification =
    numberVerified === false && Boolean(companyNumber);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setKpi({
        revenue:
          orderSummary?.data?.weekRevenue ?? summary?.data?.todaySales ?? 0,
        orders:
          orderSummary?.data?.totalOrders ?? summary?.data?.todayOrders ?? 0,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const lowStock = Array.isArray(lotRows)
        ? lotRows.filter(
            (l) => (l.availableStock ?? l.qty ?? 0) <= (l.reorderPoint ?? 0),
          ).length
        : 0;
      setCounts({
  ...
  const [dismissedLots, setDismissedLots] = useState(new Set()); // hidden from panel until reload
  ...
  const loadTransfers = () => {
    if (!imsActive) return;
    getTmsShipments()
      .then((r) => setTransfers(listOf(r)))
      .catch(() => {});
    getPendingReceiptLots()
      .then((r) => setPendingLots(listOf(r)))
      .catch(() => {});
  };
  ...
  const fromId = (s) =>
    String(s.fromWarehouseId?._id || s.fromWarehouseId || "");
  ...
  // Lots sent by company to THIS operator's warehouse, not yet received
  const myPendingLots = pendingLots.filter((l) => !dismissedLots.has(String(l._id)));

  const showTransfers = imsActive && (transferRows.length > 0 || myPendingLots.length > 0);
  ...
  const dispatchTransfer = async (s) => {
    try {
      ...
      setManifest({
        qrPayload: info?.qrPayload || `${s._id}.${s.qrToken || ""}`,
      });
      loadTransfers();
    } catch (err) {
      apiError(err);
  ...
  const cardMeta = useMemo(
    () => ({
      dashboard: {
        metric: canSeeSales
          ? kpi.revenue != null
            ? formatINR(kpi.revenue)
            : "—"
          : "Overview",
        metricLabel: canSeeSales ? "this week" : "",
      },
      inventory: {
        metric: counts.lots != null ? `${counts.lots} lots` : "—",
        pending: counts.lowStock ? `${counts.lowStock} low stock` : null,
      },
      "upload-product": { metric: "Add product" },
      "product-catalog": { metric: "Browse & edit" },
      warehouses: {
        metric: counts.warehouses != null ? `${counts.warehouses} sites` : "—",
      },
      operations: {
        metric:
          kpi.openShipments != null ? `${kpi.openShipments} shipments` : "—",
        pending: kpi.openShipments ? "in transit" : null,
      },
      labels: { metric: "Generate & print" },
      orders: {
        metric: kpi.orders != null ? `${kpi.orders} orders` : "—",
        pending: counts.pendingOrders
          ? `${counts.pendingOrders} to approve`
          : null,
      },
  ...
  const transfersPanel = (
    <div className="bg-white border border-stone-200 rounded-2xl shadow-sm mb-8 overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-stone-100">
        <span className="material-symbols-outlined text-[#EA2831]">
          sync_alt
        </span>
        <h2 className="font-bold text-stone-900">
          {isAdmin ? "Transfers" : "Transfers needing you"}
        </h2>
        <span className="text-[10px] font-bold uppercase tracking-wider text-[#EA2831] bg-[#EA2831]/10 rounded-full px-2.5 py-1">
          {transferRows.length + myPendingLots.length}
        </span>
      </div>
      <div className="divide-y divide-stone-100">
          ...
          <div key={l._id} className="flex items-center gap-3 px-5 py-3">
            <div className="min-w-0 flex-1">
              <p className="text-sm font-bold text-stone-900 truncate">
                Company <span className="text-stone-300">{'->'}</span> {l.warehouseId?.name || 'Your warehouse'}
              </p>
              <p className="text-[11px] text-stone-400">
                {l.productId?.productName || 'Product'} &middot; Lot {l.lotNumber || '-'} &middot; Qty {l.inTransitStock}
              </p>
            </div>
            <span className="shrink-0 text-[10px] font-bold px-2.5 py-1 rounded-full bg-amber-50 text-amber-700">
              Pending receipt
            </span>
            <button
            ...
            <button
              onClick={() => setDismissedLots((prev) => new Set([...prev, String(l._id)]))}
              className="shrink-0 text-[11px] font-bold text-stone-300 hover:text-[#EA2831] transition-colors px-1"
              title="Dismiss"
            >
        ...
        {transferRows.map((s) => {
          const canDispatchRow =
            isOperator &&
            myWh.includes(fromId(s)) &&
            DISPATCH_STATUSES.includes(s.status);
          const canReceiveRow =
            isOperator &&
            myWh.includes(toId(s)) &&
            RECEIVE_STATUSES.includes(s.status);
          return (
            <div key={s._id} className="flex items-center gap-3 px-5 py-3">
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold text-stone-900 truncate">
                  {s.fromLabel || "Source"}{" "}
                  <span className="text-stone-300">→</span> {s.toLabel}
                </p>
                <p className="text-[11px] text-stone-400">
                  {(s.lines || []).length} lot(s)
                </p>
              </div>
              {canDispatchRow && (
                <button
                  onClick={() => dispatchTransfer(s)}
              ...
              {canReceiveRow && (
                <button
                  onClick={() => navigate('/operations?tab=shipments')}
                  className="shrink-0 text-xs font-bold px-3 py-1.5 rounded-full border border-stone-200 text-stone-600 hover:bg-stone-50 transition-colors"
                >
                  Receive
                </button>
              ...
              {isAdmin && (
                <span
                  className={`shrink-0 text-[10px] font-bold px-2.5 py-1 rounded-full ${transferPillCls(s.status)}`}
                >
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-8 py-8">
      ...
      <div className="mb-7">
        ...
        <p className="text-stone-500 mt-1">
          Here is your business at a glance. Pick a module to get started.
        </p>
      </div>

      {/* Phone verification — greeting ke theek neeche, kyunki yeh account ki
          apni baat hai, kisi module ka kaam nahi. Wahi brand colour jo baaki
          banners ka hai. Number verify hote hi khud gayab ho jata hai; band
          karne ka button jaan-bujh kar nahi diya.

          Number khud yahan nahi chhapta — screen kisi aur ke saamne khuli ho
          sakti hai, aur user ko apna number batane ki zarurat bhi nahi.

          Message mein koi vaada nahi hai. Verified number se abhi aage kuch
          nahi hota, isliye "orders aur approvals ke liye zaruri hai" jaisi
          baat likhna galat hota — jis din number se kuch judega, tab yahin
          likh dena. */}
        ...
        <div className="mb-6 flex items-center gap-3 bg-[#EA2831]/5 border border-[#EA2831]/30 rounded-2xl p-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-stone-500">
              Verify your number to secure your account. It only takes a minute.
            </p>
          </div>
          <button
            onClick={() => navigate("/profile")}
            className="shrink-0 text-xs font-bold px-4 py-2 rounded-lg bg-[#EA2831] text-white hover:bg-[#d11f28] transition-colors"
          >
            Verify now
          </button>
        </div>
      ...
      {supplyPending > 0 && (
        <div className="mb-6 flex items-center gap-3 bg-[#EA2831]/5 border border-[#EA2831]/30 rounded-2xl p-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-stone-900 text-sm">
              {supplyPending} seller supply request
              {supplyPending > 1 ? "s" : ""} need{supplyPending > 1 ? "" : "s"}{" "}
              action
            </p>
            <p className="text-xs text-stone-500">
              Dealers are requesting bulk supply of your products — assign a
              warehouse and approve.
            </p>
          </div>
          <button
        ...
        {canSeeSales ? (
          <>
            ...
            <KpiTile
              ...
              value={formatINR(kpi.stockValue || 0)}
              accent="text-stone-900"
            />
            <KpiTile
              label="Alerts"
              value={(counts.lowStock || 0) + (kpi.alerts || 0)}
              accent="text-[#EA2831]"
            />
          </>
        ) : (
      ...
      <div className="mb-8">
        <HomeUpdates
          ...
          markRead={markRead}
          markAll={markAll}
          resolveRoute={companyNotifRoute}
          ...
          return (
            <button
              ...
              onClick={() =>
                isLocked
                  ? navigate("/billing", {
                      state: { fromKey: m.key, fromTitle: m.title },
                    })
                  : navigate(m.path)
              }
              ...
              className={`group text-left bg-white border border-stone-200 rounded-2xl p-6 shadow-sm transition-all ${
                isLocked
                  ? "opacity-90 hover:border-stone-300"
                  : "hover:shadow-md hover:border-[#EA2831]/40"
              }`}
            >
              ...
              <p className="text-sm text-stone-500 leading-snug mb-4">
                {m.description}
              </p>
              <div className="flex items-center justify-between gap-2">
                ...
                {isLocked ? (
                  <span className="material-symbols-outlined text-stone-300 shrink-0">
                    lock
                  </span>
                ) : (
      ...
      {manifest && (
        <ManifestModal info={manifest} onClose={() => setManifest(null)} />
      )}
      {receive && (
        <ReceiveModal
          shipment={receive}
...
const KpiTile = ({ label, value, accent }) => (
  ...
  <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm min-w-0">
    <p className="text-stone-400 text-[10px] font-bold uppercase mb-2 tracking-wider truncate">
      {label}
    </p>
    <p
      className={`text-xl sm:text-2xl font-bold leading-tight break-words ${accent}`}
    >
      {value}
    </p>
  </div>
