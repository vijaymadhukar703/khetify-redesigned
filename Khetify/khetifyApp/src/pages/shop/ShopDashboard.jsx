...
// Returns a translation KEY, not a string: this helper runs outside any
// component, where no t() exists. The caller resolves it.
const greetingKey = () => {
...
const TRUST = [
  { icon: "verified", titleKey: "trust.verifiedSellers", subKey: "trust.verifiedSellersSub" },
  { icon: "local_shipping", titleKey: "trust.farmDelivery", subKey: "trust.farmDeliverySub" },
  { icon: "payments", titleKey: "trust.cod", subKey: "trust.codSub" },
  { icon: "eco", titleKey: "trust.freshStock", subKey: "trust.freshStockSub" },
];
      ...
      <div className="flex flex-col justify-center gap-3 p-6 sm:gap-4 sm:p-10">
        ...
        <div className="mt-2 flex flex-col gap-3 sm:flex-row">
          ...
          >
            ...
            {inCart ? t("home.alreadyInCart") : !inStock ? t("home.soldOut") : t("home.addToCart")}
          </button>
          <Link
            to={href}
      ...
      <div className="mx-auto max-w-7xl px-3 sm:px-6">
        ...
        <section className="relative mt-4 overflow-hidden rounded-3xl bg-[#14201A] sm:mt-6 sm:rounded-[2rem]">
          ...
          <div className="relative px-5 py-10 sm:px-10 sm:py-14 lg:px-14">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-white/85 ring-1 ring-white/15 backdrop-blur">
              <span className="size-1.5 rounded-full bg-emerald-400" /> {t("dash.badge")}
            </span>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3 sm:mt-7">
              <a href="#products" className="inline-flex items-center gap-2 rounded-xl bg-[#EA2831] px-6 py-3 text-sm font-bold text-white shadow-lg shadow-black/20 transition-colors hover:bg-[#c91e26] sm:px-7 sm:py-3.5">
                {t("dash.exploreProducts")} <span className="material-symbols-outlined text-lg">arrow_forward</span>
              </a>
        ...
        <section className="mt-5 sm:mt-6">
          <div className="grid grid-cols-2 gap-2.5 sm:gap-3 lg:grid-cols-4 lg:gap-4">
            {/* The map param was named `t`, which SHADOWED the translator —
                renamed to `item` so t() resolves to the translation function. */}
            {TRUST.map((item) => (
              <div key={item.titleKey} className="flex items-center gap-3 rounded-2xl bg-white p-3 ring-1 ring-stone-200/70 sm:p-4">
                ...
                <span className="min-w-0">
                  <span className="block font-heading text-[13px] font-bold text-stone-900 sm:text-sm">{t(item.titleKey)}</span>
                  <span className="hidden truncate text-xs text-stone-500 sm:block">{t(item.subKey)}</span>
                </span>
              </div>
            ))}
          </div>
        ...
        <section className="pb-14 sm:pb-16">
          <div className="relative overflow-hidden rounded-3xl bg-[#EA2831] px-6 py-10 text-center sm:px-12 sm:py-14">
            ...
            <div className="relative">
              <h2 className="font-heading text-xl font-extrabold text-white sm:text-3xl">{t("dash.ctaTitle")}</h2>
