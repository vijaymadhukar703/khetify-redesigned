...
import {
  ...
  sellerNotifRoute,
} from "../../hooks/useSellerNotifications";
...
const SellerHub = () => {
  ...
  // Phone verification banner. Member ka handset seller account ka number nahi
  // hota, aur server uske liye likhta bhi nahi — isliye banner sirf owner ko.
  const [phoneVerified, setPhoneVerified] = useState(null);
  const [isMember, setIsMember] = useState(false);
  const needsPhoneVerification = phoneVerified === false && !isMember;

  // Identity + supplying-company link (drives the greeting + the gate banner).
  const loadLink = () => {
    getSellerLink()
      .then((r) => setLink(r?.data || null))
      .catch(() => setLink(null))
  ...
  useEffect(() => {
    ...
    getSellerMe()
      .then((r) => {
        if (!alive) return;
        setName(r?.data?.name || r?.data?.sellerInfo?.businessName || "");
        // Phone verification banner ka data — usi call se, koi naya request nahi.
        //
        // `null` ka matlab "abhi pata nahi" hai, `false` se alag: jab tak jawab
        // na aa jaye banner nahi dikhta, warna har refresh par ek pal ko chamak
        // jata — us seller ko bhi jiska number verified hai.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const inventoryValue = lotRows
        ? lotRows.reduce(
            (s, l) => s + (l.availableStock || 0) * (l.productId?.mrp || 0),
            0,
          )
        : null;
      const lowStock = lotRows
        ? lotRows.filter(
            (l) =>
              (l.lowStockThreshold || 0) > 0 &&
              (l.availableStock || 0) <= l.lowStockThreshold,
          ).length
        : 0;
      const expiring = lotRows
        ...
        : 0;
      const lotCount = lotRows
        ? lotRows.length
        : whRows.reduce((s, w) => s + (w.lotCount || 0), 0);
      const openShipments = orderRows.filter(
        (o) => !CLOSED.includes(o.status),
      ).length;
  ...
  const toReceive = useMemo(
    () => supply.filter((o) => RECEIVABLE.includes(o.status)),
    [supply],
  );
  ...
  const cardMeta = useMemo(
    () => ({
      warehouses: {
        metric:
          counts.warehouses != null ? `${counts.warehouses} site(s)` : "—",
      },
      catalog: { metric: "Browse products" },
      inbound: {
        metric: "Request & track",
        pending: toReceive.length ? `${toReceive.length} to receive` : null,
      },
      inventory: {
        metric: counts.lots != null ? `${counts.lots} lot(s)` : "—",
  ...
  const openCard = (m, planLocked) => {
    if (planLocked) {
      if (canBill) navigate("/seller/billing");
        ...
        toast(
          "info",
          "Ask your seller admin to upgrade the plan to unlock this.",
        );
      return;
    }
    navigate(m.path);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-8 py-8">
      ...
      <div className="mb-7">
        ...
        <p className="text-stone-500 mt-1">
          Here is your distribution at a glance. Pick a module to get started.
        </p>
      </div>
      ...
      {/* Phone verification — greeting ke theek neeche, kyunki yeh account ki
          ...
          sakti hai, aur seller ko apna number batane ki zarurat bhi nahi. */}
      ...
      {!linkLoading && (
        <SupplyingCompany link={link} canApply={hasCap("company:manage")} />
      )}
      ...
      {toReceive.length > 0 && (
        <div className="mt-6 mb-2 flex items-center gap-3 bg-[#EA2831]/5 border border-[#EA2831]/30 rounded-2xl p-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-stone-900 text-sm">
              {toReceive.length} supply shipment
              {toReceive.length > 1 ? "s" : ""} ready to receive
            </p>
            <p className="text-xs text-stone-500">
              Scan the shipment label to receive the stock into your warehouse.
            </p>
          </div>
          <button
            onClick={() => navigate("/seller/supply")}
            className="shrink-0 inline-flex items-center gap-1 text-xs font-bold px-4 py-2 rounded-lg bg-[#EA2831] text-white hover:bg-[#d11f28] transition-colors"
          >
      ...
      */}
      ...
      <div className="mb-8">
        <HomeUpdates
          ...
          markRead={markRead}
          markAll={markAll}
          resolveRoute={sellerNotifRoute}
          fallbackRoute="/seller/hub"
        />
      </div>
        ...
        {cards.map((m) => {
          ...
          const lockHint =
            planLocked && canBill
              ...
              : planLocked
                ? "Ask your seller admin to upgrade"
                : "";
          return (
            <button
              key={m.key}
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2">
                <span className="text-sm font-bold text-stone-700 truncate min-w-0">
                  {planLocked ? (
                    <span className="text-stone-500 font-medium">
                      {canBill ? "Upgrade to unlock" : "Ask your admin"}
                    </span>
                  ) : (
                    meta.metric
                  )}
                ...
                <span
                  className={`material-symbols-outlined shrink-0 ${isLocked ? "text-stone-300" : "text-stone-300 group-hover:text-[#EA2831] group-hover:translate-x-0.5 transition-all"}`}
                >
...
const SupplyingCompany = ({ link, canApply }) => {
  ...
  const showApprovedBanner =
    status === "approved" && !link?.linkApprovalAcknowledged && !dismissed;
  useEffect(() => {
      ...
      <div className="bg-green-50 border border-green-200 rounded-2xl p-5 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-green-700">
            You&apos;re approved — your modules are unlocked.
          </p>
        </div>
        <button
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 flex items-center gap-3">
        ...
        <div>
          <p className="font-bold text-amber-800">
            Awaiting approval from {companyName}
          </p>
          <p className="text-sm text-amber-700">
            Your portal is open in the meantime — approval is what lets you
            order and resell {companyName}&apos;s products.
          </p>
        </div>
      </div>
    );
      ...
      {status === "rejected" && (
        <div className="mb-4 bg-red-50 border border-red-200 rounded-lg p-3 text-sm">
          <p className="font-bold text-red-700">
            Your application to {companyName} was rejected.
          </p>
          {link?.linkRejectionReason && (
            <p className="text-red-600 mt-0.5">
              Reason: {link.linkRejectionReason}
            </p>
          )}
          {canApply && (
            <p className="text-red-600 mt-0.5">
              Pick another company below to re-apply.
            </p>
          )}
        </div>
      )}
      <h3 className="text-base font-bold text-stone-900 mb-1">
        Get authorized to sell
      </h3>
      <p className="text-sm text-stone-500 mb-4">
        Sellers resell a company&apos;s products. Apply for a Principal
        Certificate from the company that supplies you — once it&apos;s issued,
        you&apos;re authorized.
      </p>
      {canApply ? (
        <CompanyPicker />
        ...
        <p className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3">
          Your seller admin applies for the Principal Certificate.
        </p>
      )}
    </div>
  );
