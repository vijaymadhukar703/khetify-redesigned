...
import { changeMySellerPassword, requestSellerPasswordReset, getSellerMe } from "../../lib/sellerApi";
...
/**
 * SELLER WAREHOUSE — Account Settings.
 *
 * The company Warehouse Settings page (pages/Company/WarehouseSettings) with
 * its seller wiring: same two options, same layout, same field rules, same
 * strength meter, same success/error handling. Only three things differ, and
 * each because it has to:
 *
 *   · the SELLER permission context supplies the signed-in name
 *   · CHANGE PASSWORD posts the seller token to the SAME shared endpoint
 *     (/api/users/change-password), which is scoped to the caller's own account
 *     rather than to a company
 *   · FORGOT PASSWORD posts to /api/seller/forgot-password, because the company
 *     endpoint only matches `ownerType: "company"` users
 *
 * The company page is not touched, and nothing here is reachable from the
 * ordinary seller side — the menu entry is warehouse-role only.
 */
const SellerWarehouseSettings = () => {
  /**
   * The signed-in name comes from the seller /me endpoint, not from the
   * permission context: that context carries role and capabilities only — no
   * name — unlike the company one this page was modelled on. Fetching it here
   * keeps the change local; widening the shared context would touch every other
   * seller page for one label.
   *
   * Purely cosmetic — it fills the header chip and the email placeholder. If
   * the call fails the page still works in full; the chip simply does not show.
   */
  ...
  useEffect(() => {
    ...
    getSellerMe()
      .then((r) => {
        if (!alive) return;
        const me = r?.data || r || {};
        setName(me.name || me.businessName || "");
        setLoginEmail(me.email || "");
      })
      .catch(() => { /* cosmetic only — never blocks the forms */ });
    return () => { alive = false; };
  }, []);
  ...
  // Pre-filled with the address this account signs in with, so the common case
  // is one click. Still fully editable.
  useEffect(() => { if (loginEmail) setResetEmail((v) => v || loginEmail); }, [loginEmail]);
  ...
  const sendReset = async (ev) => {
    ...
    try {
      setResetBusy(true);
      const r = await requestSellerPasswordReset({ email });
      setResetNote(
        r?.message ||
...
export default SellerWarehouseSettings;