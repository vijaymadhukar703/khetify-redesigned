...
import { getLibraryCompanies, getLibraryProducts, getLibraryProductDetails } from '../../lib/sellerLibraryApi';
 ...
 * `disabled` locks it shut (e.g. Product Name until a company is picked);
 * `emptyText` is what an EMPTY option list says (a filter with no hits still
 * says "No match").
 */
const SearchableSelect = ({ id, value, options, onChange, placeholder, className = '', disabled = false, emptyText = 'No match' }) => {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div className="relative" ref={ref}>
      <div
        className={`${className} flex items-center justify-between gap-2 text-left transition-colors ${
          open ? 'border-[#EA2831] ring-2 ring-[#EA2831]/20' : ''
        } ${value ? 'text-stone-800' : 'text-stone-400'} ${disabled ? 'opacity-60 cursor-not-allowed' : ''}`}
      >
        <button
          type="button"
          id={id}
          ...
          aria-haspopup="listbox"
          aria-expanded={open}
          className={`flex-1 min-w-0 text-left truncate outline-none ${disabled ? 'cursor-not-allowed' : ''}`}
        >
      ...
      {open && (
        <div className="absolute z-30 mt-1.5 w-full rounded-xl border border-stone-200 bg-white shadow-lg shadow-stone-900/10">
          ...
          <ul role="listbox" ref={listRef} className="py-1.5 max-h-60 overflow-y-auto">
            {filtered.length === 0 && (
              <li className="px-3.5 py-2 text-sm text-stone-400">{options.length === 0 ? emptyText : 'No match'}</li>
            )}
            {filtered.map((opt, i) => {
...
// Inbuilt Library: "no matches", with ONE identity so effects keyed on it do
// not re-run every render.
const NO_LIB_MATCHES = [];

const BLANK = {
  // Inbuilt Library tab only. Validated on the form but NOT sent yet — the
  // backend does not accept it on seller products.
  company_name: '',
  product_name: '',
  category: '',
  categoryOther: '',
 ...
 */
const SellerLibraryProductForm = ({ productId = null, onCancel, onSaved }) => {
  ...
  /* ================= INBUILT LIBRARY NAMES =================
     Company Name and Product Name are picked from the admin Product Library
     (names only — /api/seller/library). Products are per company, so the list
     is emptied the moment the company changes (in its onChange) and refilled
     by the effect below. */
  const [libCompanies, setLibCompanies] = useState([]);
  const [libProducts, setLibProducts] = useState([]);

  useEffect(() => {
    let ignore = false;
    getLibraryCompanies()
      .then((r) => { if (!ignore) setLibCompanies(Array.isArray(r?.data) ? r.data : []); })
      .catch(() => { if (!ignore) setLibCompanies([]); });
    return () => { ignore = true; };
  }, []);
  ...
  useEffect(() => {
    ...
    let ignore = false;
    getLibraryProducts(company)
      .then((r) => {
        if (ignore) return;
        const names = (Array.isArray(r?.data) ? r.data : []).map((p) => p.productName).filter(Boolean);
        setLibProducts([...new Set(names)]);
      })
      .catch(() => { if (!ignore) setLibProducts([]); });
    // A slower answer for the PREVIOUS company must not overwrite this one.
    return () => { ignore = true; };
  ...
  /* ================= PRODUCT → FORM =================
     One mapping from a stored product to the form, shared by EDIT (hydrate from
     the seller's own product) and the INBUILT LIBRARY auto-fill (from an admin
     library product). `keep` overrides mapped fields — the library fill uses it
     to hold on to what the seller picked (company, name, category…). */
  const fillFromProduct = (p, keep = {}) => {
    ...
    if (rows.length) {
      ...
      setVariantRows(rows.map(v => { const row = hydrateSellerVariant(v); if (!productId) { delete row._id; delete row._measurementBaseline; } return row; }));
    } else {
      // No variants: clear any grid left over from a PREVIOUS library
      // fill. (On edit both are still empty here, so this is a no-op.)
      setAttrs([]);
      setVariantRows([]);
    }
  };
  ...
  /* ================= INBUILT LIBRARY: AUTO-FILL =================
     Company + Product Name + Category pick the admin library product(s); the
     one whose "Product" (horticulture) matches — or the only one — fills the
     rest of the form. Everything stays editable afterwards.

       libMatches   — the last /product-details answer for the three picks
       libHint      — the amber line under Category / Product
       autoFilled   — the form currently holds a library product's details
       lastFilledId — WHICH one, so the fill does not re-run on its own
                      echo (it writes horticulture_product, which is a dep) and
                      a seller's edits are not overwritten by the same product */
  // Each answer is stored WITH the selection it answers (`key`), so libMatches
  // is derived rather than reset in an effect:
  //   NO_LIB_MATCHES — the selection is incomplete (stable identity)
  //   null           — a lookup for the current selection is still in flight;
  //                    nothing is decided on the previous selection's answer
  //   [...]          — the answer for exactly the current selection
  const [libResult, setLibResult] = useState({ key: '', data: NO_LIB_MATCHES });
  const [libHint, setLibHint] = useState('');
  const [autoFilled, setAutoFilled] = useState(false);
  const lastFilledId = useRef(null);

  const libKey = formData.company_name && formData.product_name && formData.category
    ? JSON.stringify([formData.company_name, formData.product_name, formData.category])
    : '';
  const libMatches = !libKey ? NO_LIB_MATCHES : (libResult.key === libKey ? libResult.data : null);

  useEffect(() => {
    if (!libKey) return undefined;
    const [company, name, category] = JSON.parse(libKey);
    let ignore = false;
    getLibraryProductDetails(company, name, category)
      .then((r) => { if (!ignore) setLibResult({ key: libKey, data: Array.isArray(r?.data) ? r.data : NO_LIB_MATCHES }); })
      .catch(() => { if (!ignore) setLibResult({ key: libKey, data: NO_LIB_MATCHES }); });
    // An answer for an OLDER selection must not land on the current one.
    return () => { ignore = true; };
  }, [libKey]);

  useEffect(() => {
    if (libMatches === null) return; // still loading — decide on the real answer
    const hp = (formData.horticulture_product || '').trim();
    /* STRICT matching. The "Product" the seller picked must be the one the
       admin uploaded with this Company + Product Name + Category:
         1) exact "Product" match                       → fill it
         2) no Product picked AND exactly one candidate → fill it, and select
            its Product (the effect then re-runs on that value, finds the same
            _id and stops at the lastFilledId guard)
         3) anything else → clear an auto-filled form and say why.
       A Product that is picked but not in the library NEVER falls back to
       another product's details. */
    const exact = libMatches.find(m => (m.horticultureProduct || '').trim() === hp);
    const match = exact || (hp === '' && libMatches.length === 1 ? libMatches[0] : null);

    if (match) {
      setLibHint('');
      if (String(match._id) === lastFilledId.current) return;
      lastFilledId.current = String(match._id);
      fillFromProduct(match, {
        company_name: formData.company_name,
        product_name: formData.product_name,
        category: formData.category,
        horticulture_product: match.horticultureProduct || '',
        isActive: true,
      });
      // The library product's images arrive as keptImages; photos the seller
      // had picked for the PREVIOUS fill are dropped (and their previews freed).
    ...
    /* clearAutoFill — only a form that WAS auto-filled is cleared; a seller
       filling the form by hand is never wiped by a lookup. The seller's own
       picks (incl. the Product they chose) stay, and so do the photos they
       picked themselves (newFiles / newPreviews). */
    if (autoFilled) {
      setFormData(prev => ({
        ...BLANK,
        ...
        horticulture_product: prev.horticulture_product,
      }));
      setKeptImages([]);
      setAttrs([]);
      setVariantRows([]);
    ...
    if (hp === '' && libMatches.length > 1) {
      setLibHint('Multiple products found — select Product to load details.');
    } else if (hp !== '' && libMatches.length > 0) {
      setLibHint('No library product for this Product — fill the details yourself.');
    } else if (libMatches.length === 0 && libKey) {
      // Only for a COMPLETE selection — clearing the company (which also
      // clears Product Name) is not a failed lookup.
      setLibHint('No library product found for this selection.');
    } else {
      setLibHint('');
    }
  }, [libMatches, formData.horticulture_product]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    setVariantRows(prev =>
      combos.map(combo => {
        ...
        return {
          ...
          _id: existing?._id,
          _measurementCleared: existing?._measurementCleared,
          _variantBaseline: existing?._variantBaseline,
          ...(existing && Object.hasOwn(existing, "_measurementBaseline") ? { _measurementBaseline: existing._measurementBaseline } : {}),
  ...
  /* ================= VALIDATION ================= */
  const validate = () => {
    if (!formData.company_name.trim()) return 'Please enter the Company Name.';
    ...
    if (formData.has_variants === 'yes') {
      for (const row of variantRows) {
        const error = editMeasurementError(row);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-1">
              ...
              <SearchableSelect
                ...
                options={libCompanies}
                placeholder="Select company"
                emptyText="No companies found"
                onChange={(v) => {
                  // A new company means a new product list: clear both the
                  // picked product and the old company's options right away.
                  setLibProducts([]);
                  setFormData(prev => ({ ...prev, company_name: v || '', product_name: '' }));
                }}
              />
            </div>
            <div className="md:col-span-1">
              ...
              <SearchableSelect
                ...
                placeholder={formData.company_name ? 'Select product' : 'Select a company first'}
                emptyText="No products found"
                onChange={(v) => setFormData(prev => ({ ...prev, product_name: v || '' }))}
              />
              {!formData.company_name && (
                <p className={hintClass}>Select a Company Name first.</p>
              )}
            ...
            {/* Inbuilt Library auto-fill status — under the Category / Product
                row, full width, and only when there is something to say. */}
            {libHint && (
              <p className={`md:col-span-2 -mt-4 ${hintClass} text-amber-600`}>{libHint}</p>
            )}
                      ...
                      {variantRows.map((row, i) => (
                        ...
                        >
                          ...
                          <button type="button" className="rounded-lg border border-stone-200 p-2 text-[#EA2831] text-xl font-bold"
                            ...
                            aria-controls={'library-variant-measurements-' + i}
                            onClick={() => setExpandedVariant(current => current === row.label ? null : row.label)}>+</button>
                          {expandedVariant === row.label && (
                            <div id={'library-variant-measurements-' + i} className="col-span-full border-t border-stone-100 bg-white p-4">
                              <VariantMeasurements value={row.measurements} units={UNIT_OPTIONS} idPrefix={'library-variant-' + i}
                                ThemedSelect={ThemedSelect} inputClass={inputClass} labelClass={labelClass} hintClass={hintClass}
                                onChange={measurements => patchVariantRow(row.label, { measurements, _measurementCleared: measurements === undefined })} />
                            </div>
                          )}
                        </div>
                      ))}
...
export default SellerLibraryProductForm;