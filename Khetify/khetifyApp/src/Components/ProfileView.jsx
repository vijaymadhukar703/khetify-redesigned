...
/* 📱 PHONE VERIFICATION — company portal only.

   `phoneVerification` prop मिलने पर ही render होता है (वही तरीका जो `licences`
   का है), इसलिए seller profile ज्यों की त्यों रहती है — वो यह prop नहीं भेजती.

   Popup उसी screen पर खुलता है, नया page नहीं: verify करना profile देखने के
   बीच का एक छोटा काम है, अपनी अलग जगह नहीं माँगता. */
function PhoneVerify({
  ...
  const typed = String(currentPhone || "").trim();
  const onAccount = String(accountPhone || "").trim();
  // Badge tabhi jab dikh raha number wahi ho jo verify hua tha. Edit me koi
  // naya number type kare to "Verified" dikhana jhooth hoga — wo number abhi
  // saabit nahi hua.
  // onAccount na mile (prop hi na aaya ho) to usse badge rokna galat hai —
  // verification asli hai, hamare paas milane ko number nahi. Aisi soorat me
  // `verified` par hi bharosa karo.
  const showsVerified =
    verified && (!typed || !onAccount || typed === onAccount);
  // 10 ank poore na ho to verify ka koi matlab nahi.
  const canVerify = /^[0-9]{10}$/.test(typed || onAccount);
  ...
  const start = async () => {
    ...
    setBusy(true);
    try {
      const r = await onSendOtp(
        typed && typed !== onAccount ? typed : undefined,
      );
      setSentTo(r?.phoneMasked || phoneMasked || "");
      setOpen(true);
      setCooldown(60);
      // Jo sach hai wahi kaho. SMS na gayi to use success batana galat hai —
      // user code ka intezaar karta reh jayega.
      onToast?.(
        r?.otpSent
          ? `Verification code sent to ${r?.phoneMasked || phoneMasked || "your number"}`
          : "Could not send the SMS. Try resending in a moment.",
        r?.otpSent ? "success" : "error",
      );
    } catch (e) {
      onToast?.(
        e?.response?.data?.message || "Could not send the code",
        "error",
      );
    } finally {
      setBusy(false);
  ...
  const resend = async () => {
    ...
    try {
      ...
      setErr(e?.response?.data?.message || "Could not resend the code");
    }
  };
  ...
  const submit = async (e) => {
    ...
    try {
      await onVerify(code);
      setOpen(false);
      onToast?.("Phone number verified", "success");
    } catch (e2) {
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1 text-[11px] font-bold text-[#EA2831] bg-[#EA2831]/5 border border-[#EA2831]/30 rounded-full px-2 py-0.5 hover:bg-[#EA2831]/10 transition-colors disabled:opacity-60"
      >
        <span className="material-symbols-outlined text-[13px] leading-none">
          error
        </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl p-6">
            ...
            <p className="text-xs text-stone-500 mb-5">
              We sent a 6-digit code to {sentTo || "your number"}.
            </p>
            ...
            <form onSubmit={submit}>
              <input
                ...
                maxLength={6}
                autoFocus
                className="w-full h-14 rounded-lg border border-stone-200 px-4 text-center text-xl tracking-[0.5em] focus:outline-none focus:ring-2 focus:ring-[#EA2831]/30 focus:border-[#EA2831]"
              />
              {err && (
                <p className="text-xs text-[#EA2831] font-medium mt-2">{err}</p>
              )}

              <button
                type="submit"
                disabled={busy || code.length < 6}
                className="mt-4 w-full rounded-lg bg-[#EA2831] py-3 text-sm font-bold text-white hover:bg-[#d11f28] transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
              >
            ...
            <div className="mt-4 text-center">
              <button
                ...
                onClick={resend}
                disabled={cooldown > 0}
                className="text-xs font-semibold text-[#EA2831] hover:underline disabled:text-stone-400 disabled:no-underline disabled:cursor-not-allowed"
              >
...
const fileOk = (f) =>
  !f || (/(pdf|jpe?g|png|webp)$/i.test(f.name) && f.size <= MAX_BYTES);
...
const licenceError = (r, v) => {
  ...
  if (
    v.length < LICENCE_MIN_LEN ||
    v.length > LICENCE_MAX_LEN ||
    !LICENCE_CHARS_RE.test(v)
  )
    return LICENCE_CHARS_HINT;
  return "";
};
...
const LICENCE_ROWS = [
  {
    ...
    doc: "TAN certificate",
    numField: "tanNumber",
    fileField: "tanCertificate",
    placeholder: "MUMA12345B",
    format: (v) => cleanUpper(v, 10),
    re: /^[A-Z]{4}[0-9]{5}[A-Z]$/,
    error:
      "TAN must be 10 characters — 4 letters, 5 digits, 1 letter (e.g. MUMA12345B)",
  },

  {
    key: "gumasta",
    label: "Gumasta / Shop Act",
    doc: "Gumasta certificate",
    numField: "gumastaNumber",
    fileField: "gumastaCertificate",
    format: cleanLicence,
    re: null,
  },

  {
    key: "udyam",
    label: "Udyam",
    doc: "Udyam certificate",
    numField: "udyamNumber",
    fileField: "udyamCertificate",
  ...
  {
    key: "agriculture",
    label: "Agriculture licence",
    doc: "Agriculture certificate",
    numField: "agricultureNumber",
    fileField: "agricultureCertificate",
    format: cleanLicence,
    re: null,
  },

  {
    ...
    doc: "Horticulture certificate",
    numField: "horticultureNumber",
    fileField: "horticultureCertificate",
    format: cleanLicence,
    re: null,
  },
];
...
const rowsFor = (keys) =>
  keys.map((k) => LICENCE_ROWS.find((r) => r.key === k)).filter(Boolean);
...
const licenceFileOk = (f) =>
  !f || (/(pdf|jpe?g|png)$/i.test(f.name) && f.size <= LICENCE_MAX_BYTES);
...
const DocLinks = ({ url, fileName }) => {
  ...
  if (!href)
    return <span className="text-xs text-stone-400">Not provided</span>;
  return (
...
const Empty = () => (
  <span className="text-sm text-stone-400 italic">Not provided</span>
);
...
// `after` (optional) label ke baad baithta hai — abhi sirf phone ka verified
// badge use karta hai. Omit karne par field bilkul pehle jaisi rehti hai.
const IdField = ({
  label,
  value,
  editing,
  ...
  <div className="flex flex-col gap-1">
    ...
    {error && (
      <span className="text-[11px] font-medium text-[#EA2831]">{error}</span>
    )}
  </div>
);
  ...
  <div className="flex flex-wrap items-start justify-between gap-3 py-3.5">
    <div className="min-w-[220px] flex-1">
      ...
      {editing ? (
        ...
        <p className="text-sm font-mono font-medium text-stone-800">{value}</p>
      ) : (
        <Empty />
      )}
      {error && (
      ...
      {!error && warning && (
        <span className="text-[11px] font-medium text-amber-600">
          {warning} — saved as-is; edit it to correct it
        </span>
      )}
      ...
      {editing && (
        <div className="mt-1">
          ...
          <label
            ...
            className="inline-flex items-center gap-1 text-xs font-bold text-stone-600 border border-stone-200 rounded-lg px-2.5 py-1.5 cursor-pointer hover:bg-stone-50"
          >
          ...
          {fileErr && (
            <span className="block text-[11px] font-medium text-[#EA2831] mt-0.5">
              {fileErr}
            </span>
          )}
        </div>
...
const ProfileView = ({
    ...
    return (
      <div className="flex-1 p-4 sm:p-8 bg-white font-sora">
        <div className="max-w-3xl mx-auto bg-red-50 border border-red-200 rounded-2xl p-6 text-center">
          <span className="material-symbols-outlined text-[#EA2831] text-3xl">
            error
          </span>
  ...
  const startEdit = () => {
    ...
    setSeeded(
      Object.fromEntries(
        LICENCE_ROWS.map((r) => [r.numField, lic[r.key]?.number || ""]),
      ),
    );
    setForm({
  ...
  const pickFile = (k) => (f) => {
    setErrs((e) => ({
      ...e,
      [k]: fileOk(f) ? undefined : "PDF or image up to 10MB",
    }));
  ...
  const pickOthers = (list) => {
    ...
    setErrs((e) => ({
      ...e,
      otherDocs: bad
        ? "Each file must be a PDF or image up to 10MB"
        : undefined,
    }));
    setFileNames((n) => ({
      ...n,
      otherDocs: arr.map((f) => f.name).join(", "),
    }));
  ...
  const validate = () => {
    ...
    if (form.gstin && !GSTIN_RE.test(form.gstin.trim()))
      e.gstin = "Invalid GSTIN (15 chars)";
    if (form.pan && !PAN_RE.test(form.pan.trim()))
      e.pan = "Invalid PAN (10 chars)";
    if (!fileOk(files.gstCertificate))
      e.gstCertificate = "PDF or image up to 10MB";
    if (!fileOk(files.panFile)) e.panFile = "PDF or image up to 10MB";
    if ((files.otherDocs || []).some((f) => !fileOk(f)))
      e.otherDocs = "Each file must be a PDF or image up to 10MB";
    const w = {};
    if (showLicences) {
      LICENCE_ROWS.forEach((r) => {
        if (!licenceFileOk(files[r.fileField]))
          e[r.fileField] = "PNG, JPG or PDF up to 5MB";
        ...
        if (
          LENIENT_WHEN_UNCHANGED.includes(r.key) &&
          v === (seeded[r.numField] || "").trim()
        )
          w[r.numField] = msg;
  ...
  const save = async () => {
    ...
    try {
      ...
      setBanner({
        ok: false,
        msg: err?.response?.data?.message || err.message || "Could not save",
      });
    } finally {
      setSaving(false);
    }
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center justify-between gap-3">
          ...
          {onSave && !editing && (
            <button
              onClick={startEdit}
              className="inline-flex items-center gap-1.5 border border-stone-200 hover:bg-stone-50 text-stone-700 text-sm font-bold rounded-lg px-4 py-2 transition-colors"
            >
        ...
        {banner && (
          <div
            className={`rounded-xl px-4 py-3 text-sm font-medium ${banner.ok ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-200"}`}
          >
            {banner.msg}
          </div>
        )}
        ...
        <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            ...
            <span
              ...
              style={{ color: pct === 100 ? "#16a34a" : brand }}
            >
              {pct === 100
                ? "All set 🎉"
          ...
          <div className="h-2 w-full bg-stone-100 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${pct}%`,
                backgroundColor: pct === 100 ? "#16a34a" : brand,
              }}
            />
          </div>
          {missing.length > 0 && (
        ...
        <Card title="Business identity" icon="badge">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
            <IdField
              ...
              value={editing ? form.businessName : id.businessName}
              editing={editing}
              onChange={set("businessName")}
            />
            <IdField
              label="Contact person"
              value={editing ? form.contactPerson : id.contactPerson}
              editing={editing}
              onChange={set("contactPerson")}
            />
            <IdField
              label="Email"
              value={editing ? form.email : id.email}
              editing={editing}
              onChange={set("email")}
              error={errs.email}
            />
            <IdField
              label="Phone"
              value={editing ? form.phone : id.phone}
              editing={editing}
              onChange={set("phone")}
              error={errs.phone}
              after={
                /* Sirf company portal bhejta hai; seller nahi, isliye uski
                   profile bilkul pehle jaisi rehti hai.

                   Edit me bhi dikhta hai, kyunki number badal kar verify karna
                   ek jayaz kaam hai: code naye number par jata hai aur sahi
                   nikalne par wahi account ka number ban jata hai. */
                phoneVerification ? (
                  <PhoneVerify
                    verified={phoneVerification.verified}
                    phoneMasked={phoneVerification.phoneMasked}
                    accountPhone={phoneVerification.accountPhone}
                    currentPhone={editing ? form.phone : id.phone}
                    onSendOtp={phoneVerification.onSendOtp}
                    onVerify={phoneVerification.onVerify}
                    onToast={phoneVerification.onToast}
                  />
                ) : null
              }
            />
            <div className="sm:col-span-2">
              <IdField
                ...
                value={editing ? form.address : id.address}
                editing={editing}
        ...
        <Card title="Compliance & registration" icon="verified_user">
          <div className="divide-y divide-stone-100">
            <ComplianceRow
              ...
              value={editing ? form.gstin : c.gstin}
              editing={editing}
              onChange={set("gstin")}
              error={errs.gstin}
              url={c.gstCertificateUrl}
              fileName="gst-certificate"
              fileKey="gstCertificate"
              onFile={pickFile("gstCertificate")}
              fileErr={errs.gstCertificate}
              chosenName={fileNames.gstCertificate}
            />
            <ComplianceRow
              label="PAN"
              docLabel="PAN card / file"
              value={editing ? form.pan : c.pan}
              editing={editing}
              onChange={set("pan")}
              error={errs.pan}
              url={c.panFileUrl}
              fileName="pan-file"
              fileKey="panFile"
              onFile={pickFile("panFile")}
              fileErr={errs.panFile}
              chosenName={fileNames.panFile}
            />
              ...
              {documents.map((d) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    {d.status && (
                      <span
                        className={`inline-block mt-1 text-[10px] font-bold rounded-full px-2 py-0.5 capitalize ${
                          d.status === "verified"
          ...
          {editing && (
            <div className="mt-4 border-t border-stone-100 pt-4">
              <input
                ...
                multiple
                className="hidden"
                onChange={(e) => pickOthers(e.target.files)}
              />
              <label
                htmlFor="file-otherDocs"
                className="inline-flex items-center gap-1 text-xs font-bold text-stone-600 border border-stone-200 rounded-lg px-2.5 py-1.5 cursor-pointer hover:bg-stone-50"
              >
        ...
        {editing && (
          <div className="flex items-center justify-end gap-3">
            <button
              onClick={cancel}
              disabled={saving}
              className="border border-stone-200 hover:bg-stone-50 text-stone-700 text-sm font-bold rounded-lg px-5 py-2.5 transition-colors disabled:opacity-40"
            >
              Cancel
            </button>
            <button
              onClick={save}
              disabled={saving}
              className="inline-flex items-center gap-2 bg-[#EA2831] hover:bg-[#c91e26] disabled:opacity-40 text-white text-sm font-bold rounded-lg px-5 py-2.5 transition-colors"
            >
