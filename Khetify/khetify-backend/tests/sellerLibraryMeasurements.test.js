...
test('unrelated edit preserves the existing dimensions representation', async () => {
  ...
  expect(payload[0].dimensions).toEqual(saved);
  ...
  expect(reopened.body.data.variants[0].dimensions).toEqual(saved);
  expect(reopened.body.data.variants[0].measurements).toBeUndefined();
});
