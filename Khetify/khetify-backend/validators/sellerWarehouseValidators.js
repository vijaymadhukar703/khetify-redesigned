...
/**
 * SELLER warehouse creation now carries the Warehouse Manager's details,
 * because a seller team member is created ONLY as part of creating the
 * warehouse they run — the same rule the company side already follows
 * (validators/warehouseValidators.js).
 *
 * The manager field rules are IMPORTED from userValidators.js rather than
 * restated, so the company "Add Warehouse", the seller "Add Warehouse" and the
 * old "Add Team Member" can never drift apart on what counts as a valid
 * name / email / phone / password.
 *
 * This schema is now field-for-field equivalent to the company one
 * (validators/warehouseValidators.js): address line1 + pincode mandatory, the
 * Google Maps link OPTIONAL, and the manager block required.
 *
 ...
 * below — address, location, mapsUrl, capacityUnits and geofenceRadiusM
 * included.
 */
...
// the form already collects them and `district` remains optional.
...
// OPTIONAL. The Add Warehouse form does not ask for coordinates — it takes the
// Google Maps link below instead. This schema is kept so any programmatic
// caller that already sends `location` keeps working, and so the controller can
// fill it in when it can read coordinates out of the link.
...
//
// Identical to the company rule in validators/warehouseValidators.js — the two
// schemas now agree on this field as well.
const mapsUrlBody = z
  .string()
  .trim()
...
const createSellerWarehouseBody = z.object({
  name: z
  ...
  geofenceRadiusM: z.coerce.number().nonnegative().optional(),
  manager: managerBody,
});

module.exports = { createSellerWarehouseBody };